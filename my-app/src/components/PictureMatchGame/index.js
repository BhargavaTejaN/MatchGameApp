...
import PictureMatchCard from "../PictureMatchCard/index";
...
class PictureMatchGame extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.startTick, 1000);
  }
  ...
  getActiveTabPictures = () => {
    const { activeTabId } = this.state;
    const tabImgResults = imagesList.filter(
      (eachImage) => eachImage.category === activeTabId
    );
    return tabImgResults;
  };

  onClickMatchingImage = (id) => {
    const { topScore, randomImage } = this.state;
    const isMatched = randomImage.id === id;
    if (isMatched) {
      this.setState({
        topScore: topScore + 1,
  ...
  renderPicturesList = () => {
    ...
    const filteredPicturesList = this.getActiveTabPictures();

    return (
      <div className="image-container">
        <img src={randomImage.imageUrl} alt="match" className="img-display" />
        ...
        <div className="match-body-game">
          <ul className="pictures-list">
            {filteredPicturesList.map((eachImage) => (
              <PictureMatchCard
                onClickMatchingImage={this.onClickMatchingImage}
                details={eachImage}
                key={eachImage.id}
              />
            ))}
  ...
  render() {
    const { topScore, timeElapsedInSeconds, isTimerRunning } = this.state;
    ...
    return (
      <div className="app-container">
        ...
        <div className="container-item">
          {isTimerRunning ? this.renderPicturesList() : this.renderGameOver()}
        </div>
      </div>
    );
  }
